...
static int16_t   frente;      // posicion del limite Verde|Azul (0..NUM_LEDS-1)
...
void nuevoTug() {
  frente    = NUM_LEDS / 2;   // el frente arranca en el centro
  empujones = 0;
  ganador   = 0;
  esRecord  = false;
...
void loopTug() {
  if (estadoTug == TUG_JUGANDO) {
    ...
    if      (frente >= NUM_LEDS - 1) terminar(1);   // Verde conquisto la tira
    else if (frente <= 0)            terminar(2);   // Azul conquisto la tira

    // Render: Verde [0, frente), Azul [frente, fin).
    for (int16_t i = 0; i < NUM_LEDS; i++) leds[i] = (i < frente) ? COL_P1 : COL_P2;
    FastLED.show();
...
void lcdTug() {
  ...
  uint8_t p = map(frente, 0, NUM_LEDS - 1, 0, 15);
  String bar;
...
String webTug() {
  return "Frente en el LED " + String(frente) + " de " + String(NUM_LEDS);
}
